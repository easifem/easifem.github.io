...
PROGRAM main
    ...
    CALL obj%get(value=value, globalNode=arange(1,3,1), ivar=1, idof=1)
    CALL OK( ALL(value(1:3) .APPROXEQ. svalue(1:3)), " test 1 " )

    CALL obj%get(value=value, globalNode=arange(1,3,1), ivar=1, idof=2)
    CALL OK( ALL(value(1:3) .APPROXEQ. svalue(tnodes+1:tnodes+3)), " test 2 " )
