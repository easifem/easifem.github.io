...
`dgssvx` is used to solve and factored the matrix We solve a transpose system
...
! CALL dPrint_CompCol_Matrix(CString("A before "), sluA)
...
CALL dPrint_Dense_Matrix(CString("B before "), sluB)

rhs = 2.0_DFP

CALL dPrint_Dense_Matrix(CString("B after "), sluB)
STOP
...
IF (info .EQ. 0) THEN
  ...
  CALL Display(sluR, "sluR = ")
  CALL Display(sluC, "sluC = ")
