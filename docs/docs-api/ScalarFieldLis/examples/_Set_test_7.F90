...
USE ScalarField_Class, ONLY: SetScalarFieldParam, ScalarField_
...
BLOCK
  ...
  TYPE(FEDOF_) :: fedof
  TYPE(ScalarFieldLis_) :: obj, VALUE
  ...
  CALL OK(isok, msg)
  IF (.NOT. isok) THEN
    CALL Display(msg="found", val=found(1:tsize))
    CALL Display(msg="want", val=want(1:tsize))
  END IF
...
BLOCK
  ...
  CALL SetScalarFieldParam(param=param, &
                           fieldType=TypeField%normal, &
                           name="U", &
                           engine=TypeEngineName%native_serial)
  CALL VALUE%Initiate(param, fedof)
