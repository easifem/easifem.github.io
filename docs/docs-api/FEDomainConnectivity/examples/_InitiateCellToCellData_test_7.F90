...
CHARACTER(*), PARAMETER :: myname = "InitiateCellToCellData test 7"

TYPE(FEDomainConnectivity_) :: obj
TYPE(FEMesh_) :: mesh2, mesh1
...
CALL mesh1%Initiate(domain1_file, dim=nsd)
CALL mesh2%Initiate(domain2_file, dim=nsd)

CALL obj%InitiateCellToCellData(mesh1=mesh1, mesh2=mesh2)

cellToCell => obj%GetCellToCellPointer()

! CALL Display(celltoCell, "cellToCEll", full=.TRUE.)

telems = mesh1%GetTotalElements()
...
DO iel = 1, telems
  ...
  isok = mesh2%isElementPresent(globalElement=ii, islocal=.FALSE.)
  IF (.NOT. isok) THEN
    CALL Display(myname)
