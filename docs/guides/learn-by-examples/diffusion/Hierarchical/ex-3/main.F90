...
!
!
! third order Hierarchical basis functions are used
...
CHARACTER(*), PARAMETER :: meshfilename = "./mesh/big_mesh.h5", &
...
LOGICAL(LGT), PARAMETER :: relativeToRHS = .FALSE., debug = .FALSE., &
                           isexpand = .TRUE.
...
IF (debug) CALL e%setQuietMode(EXCEPTION_INFORMATION, .TRUE.)
...
CALL geofedof%Initiate(baseContinuity="H1", &
                       baseInterpolation="Lagrange", &
                       order=order, mesh=cellmesh, ipType=ipType)
IF (debug) CALL Display("main.F90: Initiate geofedof")
IF (debug) CALL geofedof%Display("geofedof")

maxcon = fedof%GetMaxTotalConnectivity()
CALL Reallocate(cellcon, maxcon)
CALL Reallocate(ke, maxcon, maxcon)
...
DO iel = 1, telements
  ...
  IF (debug) CALL Display("main.F90: fedof GetLocalElemShapeData")

  CALL geofedof%GetLocalElemShapeData(globalElement=iel, elemsd=geoelemsd, &
                                      quad=quad, islocal=.TRUE.)

  IF (debug) CALL Display("main.F90: geofedof GetLocalElemShapeData")
  ...
  CALL fedof%GetGlobalElemShapeData(globalElement=iel, elemsd=elemsd, &
                                 xij=xij, islocal=.TRUE., geoelemsd=geoelemsd)
